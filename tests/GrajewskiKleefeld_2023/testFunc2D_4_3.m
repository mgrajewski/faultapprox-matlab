% Function for Test problem 4.3 in "Detecting and approximating decision
% boundaries in low dimensional spaces", section 4.1.
% It is based on Example 5 from Allasia, Giampietro et al., Efficient
...
function ClassOfPoints = testFunc2D_4_3(PointSet, ProblemDescr)
    
    ClassOfPoints = ones(size(PointSet,1),1);
    ClassOfPoints(PointSet(:,1) >= 0.4 & PointSet(:,2) >= 0.4 & PointSet(:,2) <= PointSet(:,1) + 0.2) = 2;
