        ...
        if(distNext(ipoint) > maxDistForSurfacePoints)
            ...
            if ndim == 2
                if ipoint > 2 && ipoint < nTriplets-2
                    curv = computeCurvature(0.5*(PointsIclass(ipoint-2:ipoint+2,:) + ...
                ...
                elseif ipoint <= 2
                    if (nTriplets > 2)
                        curv = safetyFactor*computeCurvature(0.5*(PointsIclass(1:min(nTriplets,5),:) + ...
                ...
                elseif ipoint >= nTriplets-2
                    curv = safetyFactor*computeCurvature(0.5*(PointsIclass(max(1, nTriplets-4):nTriplets,:) + ...
                                                              PointsJclass(max(1, nTriplets-4):nTriplets,:)), ...
                                                         4, abstolBisection);
                end
            else
                if ipoint > 2 && ipoint < nTriplets-2
                    curv = computeCurvature(0.5*(PointsIclass(ipoint-2:ipoint+2,~NormalVecOfPlane) + ...
                                                 PointsJclass(ipoint-2:ipoint+2,~NormalVecOfPlane)), ...
                                            3, abstolBisection);
                elseif ipoint <= 2
                    if (nTriplets > 2)
                        curv = safetyFactor*computeCurvature(0.5*(PointsIclass(1:min(nTriplets,5),~NormalVecOfPlane) + ...
                ...
                elseif ipoint >= nTriplets-2
                    curv = safetyFactor*computeCurvature(0.5*(PointsIclass(max(1,nTriplets-4):nTriplets,~NormalVecOfPlane) + ...
