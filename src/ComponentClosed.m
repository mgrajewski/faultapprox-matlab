...
% point to prevent endless loops in expand2D. We test if any point on a
% remote part of the fault line is in fact close to the last point. To do
% so, we exploit that the points on the fault line are already ordered. We
% take the first or last half of the points on the fault line depending on
% if we expand the start or the end of the fault line. We compute their
% distance to the last point. In (sub)algorithm expand, this is the point
% added at last. If this distance is small, we conclude that the fault line
% must be closed.
% This does not make sense for boundary components consisting of 6 points
% only or even less.
%
% Input:
...
% - mode: Mode of operation in expand (0: prolongate the current
...
function closedComp = ComponentClosed(Points, IdxPoints, distVec, npoints, mode)
